...
M.hsl_regex = "hsla?[(]+"
	...
	.. "[%s,/]?%s*%d*%.?%d*%%?%s*[)]+"
M.custom_hsl = "%d+%.?%d*d?e?g?t?u?r?n?%s*[,%s]%s*%d+%.?%d*%%%s*[,%s]%s*%d+%.?%d*%%"
...
---Checks whether a color is custom hsl
---@return boolean
function M.is_custom_hsl(color)
	return string.match(color, M.custom_hsl)
end
